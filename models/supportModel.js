...
class SupportTicket {
    ...
    static async getAll() {
        const [rows] = await db.execute('SELECT t.*, u.name as user_name FROM support_tickets t JOIN users u ON t.user_id = u.id');
        return rows;
    }
...
class Event {
    ...
    static async getAll() {
        const [rows] = await db.execute('SELECT e.*, c.business_name as client_name, u.name as manager_name FROM events e LEFT JOIN clients c ON e.client_id = c.id LEFT JOIN users u ON e.manager_id = u.id');
        return rows;
    }
...
class GuestRequest {
    ...
    static async getAll() {
        const [rows] = await db.execute('SELECT r.*, u.name as client_name FROM guest_requests r LEFT JOIN users u ON r.client_id = u.id');
        return rows;
    }
