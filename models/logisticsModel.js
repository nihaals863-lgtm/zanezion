...
class Vehicle {
    ...
    static async updateStatus(id, status) {
        await db.execute('UPDATE vehicles SET status = ? WHERE id = ?', [status, id]);
    }
}
...
class Delivery {
    static async create(data) {
        ...
        try {
            ...
            if (vehicle_id) {
                await connection.execute('UPDATE vehicles SET status = ? WHERE id = ?', ['on_mission', vehicle_id]);
            }
    ...
    static async updateStatus(id, status, proofData = {}) {
        ...
        if (['delivered', 'Delivered', 'Completed'].includes(status)) {
            ...
            if (deliveryRows[0] && deliveryRows[0].vehicle_id) {
                await db.execute('UPDATE vehicles SET status = ? WHERE id = ?', ['available', deliveryRows[0].vehicle_id]);
            }
        }
...
class DeliveryPricing {
    static async getAll() {
        const [rows] = await db.execute('SELECT * FROM delivery_pricing ORDER BY min_distance ASC');
        return rows;
    }
